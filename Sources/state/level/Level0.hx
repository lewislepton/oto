...
class Level0 extends State {
  ...
  public function new(setLevel:String, nextLevel:String){
    ...
    txtEnter = new Text('Abel', 'E', 0, 0, 64);
    ...
    // EXIT
		for (exits in level.exits.entities){
			if (level.enemyCount <= 0){
				exits.unlocked = true;
        txtEnter.position.x = exits.position.x + 16;
        txtEnter.position.y = exits.position.y - 64;
        if (MenuState.btnGamepad.isOn){
          if (Collide.overlap(player, exits)){
            txtEnter.color = Color.Black;
            txtEnter.string = 'X';
          } else {
            txtEnter.color = Color.fromFloats(0,0,0,0);
          }
        } else {
          if (Collide.overlap(player, exits)){
            txtEnter.color = Color.Black;
